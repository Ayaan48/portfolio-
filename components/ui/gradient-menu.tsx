    ...
    <ul className="flex gap-6">
      {menuItems.map(({ title, icon, gradientFrom, gradientTo }, idx) => (
        <li
          ...
          className="relative w-[38px] h-[38px] bg-white shadow-lg rounded-full flex items-center justify-center transition-all duration-500 hover:w-[110px] hover:shadow-none group cursor-pointer"
        >
          ...
          {/* Title */}
          <span className="absolute text-white uppercase tracking-wide text-[10px] transition-all duration-500 scale-0 group-hover:scale-100 delay-150">
            {title}
          </span>
        </li>
      ))}
